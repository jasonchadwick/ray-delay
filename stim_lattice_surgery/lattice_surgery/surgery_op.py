...
class SurgeryOp():
    ...
    def get_stim(self, init_basis: str = 'Z', observable_basis: str = 'Z', dm: int = None, meas_mode=None, expect_bell: bool = False) -> stim.Circuit:
        '''
        ...
        if meas_mode == "01TL":
            # we will define three observables so that we can assess the X0, X1, and TL error rates:
            # O0: logical Z observable for q0
            # O1: logical Z observable for q1
            # O2: all intermediate patch stabilizers
            
            for i, data_patch in enumerate(self.data_patches):
                # What does frame_changes do?
                circ.append(
                'OBSERVABLE_INCLUDE', 
                [self.patch_collection.get_meas_rec(-1, data.idx) 
                    for data in (
                        data_patch.logical_z_qubits if observable_basis == 'Z' 
                        else data_patch.logical_x_qubits)], 
                i)
            
            # now, let's build observable O2
            o2 = []
            for i, data_patch in enumerate(self.data_patches):
            ...
            circ.append('OBSERVABLE_INCLUDE', o2, 2)
        elif meas_mode == "zlike":
            # our observable will be the joint XX observable for both qubits
            # this will allow us to check for Z-like errors
            
            
            xx_observable = []
            for i, data_patch in enumerate(self.data_patches):
                qubitx = [self.patch_collection.get_meas_rec(-1, data.idx) 
                    for data in (
                        data_patch.logical_x_qubits if observable_basis == 'Z' 
                        else data_patch.logical_z_qubits)]

                xx_observable += qubitx
            
            # interestingly, we also need to include the MZ value of the qubit
            
            circ.append('OBSERVABLE_INCLUDE', xx_observable, 0)
        elif meas_mode == "single":
            pass
        elif meas_mode == "q0":
            # we want to simply check whether q0's value is appropriate, so we just need to check its logical observable
            data_patch = self.data_patches[0]
            
            frame_changes = [stim.target_rec(observable_change[1][meas.idx]) 
                                    for observable_change in data_observable_changes[data_patch]
                                    for meas in observable_change[2] if observable_change[0] == observable_basis]
            # What does frame_changes do?
            assert len(frame_changes) == 0, "Don't know how this code works so I'm not sure if this is correct."
            
            # note frame_changes removed from below code
            circ.append(
            'OBSERVABLE_INCLUDE', 
            [self.patch_collection.get_meas_rec(-1, data.idx) 
                for data in (
            ...
            # "" with q1
            data_patch = self.data_patches[1]
            ...
            # check timelike errors by seeing the region in merged space
            # to get the merged space, 
            pass
        elif meas_mode == "gl":
            # check global errors
            # for ZZ lattice surgery, this is a Z error
            # so it will be detectable by the logical X observable
            
            patch0 = self.data_patches[0]
            patch1 = self.data_patches[1]
            
            x0 = [self.patch_collection.get_meas_rec(-1, data.idx) for data in (patch0.logical_z_qubits if observable_basis == 'X'  else patch0.logical_x_qubits)]
            x1 = [self.patch_collection.get_meas_rec(-1, data.idx) for data in (patch1.logical_z_qubits if observable_basis == 'X'  else patch1.logical_x_qubits)]
            
            circ.append('OBSERVABLE_INCLUDE', x0 + x1, 0)
        else:
            if expect_bell:
